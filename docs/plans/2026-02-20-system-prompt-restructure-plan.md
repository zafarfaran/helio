...
**Goal:** Restructure the Helio system prompt with XML tags, add scenario confirmation protocol, and add feasibility rules so the agent doesn't suggest infeasible strategies or run scenarios without adviser approval.

**Architecture:** Merge the base prompt (`docs/system_prompt.md`) and `_TOOL_INSTRUCTIONS` (from `system_prompt.py`) into a single XML-structured `.md` file. Simplify the Python loader to just load + inject client context.

**Tech Stack:** Markdown with XML tags (prompt), Python (loader)

---

### Task 1: Rewrite `docs/system_prompt.md` with XML structure

**Files:**
- Modify: `docs/system_prompt.md` (full rewrite)

**Step 1: Replace the entire file content**

Replace `docs/system_prompt.md` with the following XML-structured prompt. This merges all content from the current `.md` file AND the `_TOOL_INSTRUCTIONS` string from `system_prompt.py`:
...
Run: `wc -l docs/system_prompt.md`
...
git commit -m "refactor: restructure system prompt with XML tags, scenario protocol, and feasibility rules"
```

---

### Task 2: Simplify `system_prompt.py` — remove `_TOOL_INSTRUCTIONS`

**Files:**
- Modify: `helio/apps/api/app/services/system_prompt.py:10-139` (remove _TOOL_INSTRUCTIONS) and line 178 (remove append)

**Step 1: Remove the `_TOOL_INSTRUCTIONS` constant**

Delete the entire `_TOOL_INSTRUCTIONS` string (lines 21-133 of `system_prompt.py`). This is the block starting with `_TOOL_INSTRUCTIONS = """` and ending with the closing `"""`.

**Step 2: Remove the line that appends tool instructions**

In `build_system_prompt()`, remove these two lines (around lines 177-178):
```python
    # Always include engine tool instructions so Claude never computes tax itself
    prompt += _TOOL_INSTRUCTIONS
```

The function should go directly from `prompt = base.replace(...)` to the logging block.

**Step 3: Update `_FALLBACK_PROMPT` to include minimal XML structure**

Replace the fallback prompt with a minimal version that includes the key XML tags:
...
Run: `python -c "import ast; ast.parse(open('helio/apps/api/app/services/system_prompt.py').read()); print('OK')"`
Expected: `OK`
...
git commit -m "refactor: remove _TOOL_INSTRUCTIONS from Python loader — now fully in system_prompt.md"
```

---

### Task 3: Smoke test — verify prompt builds correctly

**Files:**
- None (read-only verification)

**Step 1: Run a quick Python check that the prompt builds without errors**

Run:
```bash
cd helio/apps/api && python -c "
from app.services.system_prompt import build_system_prompt
prompt = build_system_prompt()
assert '{{CLIENT_CONTEXT}}' not in prompt, 'Client context placeholder not replaced'
assert '<role>' in prompt, 'Missing <role> XML tag'
assert '<scenario_protocol>' in prompt, 'Missing <scenario_protocol> XML tag'
assert '<feasibility_rules>' in prompt, 'Missing <feasibility_rules> XML tag'
assert '<constraints>' in prompt, 'Missing <constraints> XML tag'
assert '_TOOL_INSTRUCTIONS' not in prompt, 'Old tool instructions leaked into prompt'
print(f'Prompt built successfully ({len(prompt)} chars)')
print('All assertions passed')
"
```
Expected: `Prompt built successfully (NNNN chars)` followed by `All assertions passed`

**Step 2: Test with mock client context**
...
from app.services.system_prompt import build_system_prompt
ctx = {
    'client': {'first_name': 'James', 'last_name': 'Mitchell', 'region': 'england', 'employment_status': 'employed', 'number_of_children': 2, 'claims_child_benefit': True},
    'tax_profile': {'tax_year': '2025/26', 'total_income': 125000, 'adjusted_net_income': 112000, 'total_tax': 38000, 'effective_rate': 30.4, 'marginal_rate': 60, 'pa_status': 'tapered', 'in_pa_taper_zone': True}
}
prompt = build_system_prompt(client_context=ctx)
assert 'James Mitchell' in prompt, 'Client name not injected'
assert '<client_context>' in prompt, 'Missing client_context tag'
assert 'SUGGEST THEN CONFIRM' in prompt, 'Missing scenario protocol'
print(f'Prompt with context built successfully ({len(prompt)} chars)')
print('All assertions passed')
"
```
Expected: `Prompt with context built successfully (NNNN chars)` followed by `All assertions passed`
